...
<xsl:template match="/">
		...
		<p class="eventinfo_date">
			...
			<span>
				...
				<xsl:if test="string($currentPage/eventEndDateTime) != ''">
					<xsl:text> - </xsl:text>
					<xsl:value-of select="umbraco.library:FormatDateTime($currentPage/eventEndDateTime, 'dddd, d MMMM, yyyy')" />
				</xsl:if>
			</span>
		</p>
	...
	<xsl:if test="string($currentPage/eventLocationAddress) != '' or string($currentPage/eventLocationName) != ''">
		<p class="eventinfo_location"><strong><xsl:text>Location: </xsl:text></strong><xsl:value-of select="$currentPage/eventLocationName" /> <xsl:value-of select="$currentPage/eventLocationAddress" /></p>
	</xsl:if>
