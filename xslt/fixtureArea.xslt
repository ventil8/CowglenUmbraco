...
<xsl:template match="/">
    ...
    <div class="event_main">
      ...
      <h2>
        <xsl:value-of select="$currentPage/pageHeading"/>: <xsl:value-of select="$MonthName" /><xsl:text> </xsl:text><xsl:value-of select="$Year" />
      </h2>
      <div class="event_nav">
        <p>
          <xsl:text>Currently viewing: </xsl:text>
      ...
      <xsl:choose>
        <xsl:when test="count($currentPage//* [@isDoc and name()='Fixture' and Exslt.ExsltStrings:lowercase($currentMonthYear) = Exslt.ExsltStrings:lowercase(umbraco.library:FormatDateTime(./eventStartDateTime, 'MMMM,yyyy'))]) &gt; 0">
          <ul class="events_list">
            <xsl:for-each select="$currentPage//* [@isDoc and name()='Fixture' and Exslt.ExsltStrings:lowercase($currentMonthYear) = Exslt.ExsltStrings:lowercase(umbraco.library:FormatDateTime(./eventStartDateTime, 'MMMM,yyyy'))]">
              <xsl:sort select="current()/eventStartDateTime"  order="ascending" />
              <li>
                <p class="event_date_cal">
                  <span class="event_date_cal_day">
                    <xsl:value-of select="umbraco.library:FormatDateTime(current()/eventStartDateTime, 'ddd')"/>
                  </span>
                  <span class="event_date_cal_month">
                    <xsl:value-of select="umbraco.library:FormatDateTime(current()/eventStartDateTime, ' d')"/>
                  </span>
                </p>
                <h3>
                  <a href="{umbraco.library:NiceUrl(current()/@id)}">
                    <xsl:value-of select="current()/@nodeName" /> - <xsl:value-of select="current()/homeTeam" /> V <xsl:value-of select="current()/awayTeam" /> (<xsl:value-of select="current()/ageGroup" />'s)
                  </a>
                </h3>
                <p>
        ...
        <xsl:otherwise>
          <p class="no_events">There are no fixtures this month</p>
        </xsl:otherwise>
      </xsl:choose>
    </div>
	</div>
